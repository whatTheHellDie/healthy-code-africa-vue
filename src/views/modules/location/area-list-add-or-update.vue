<template>
  ...
  <div class="mod-demo-ueditor">
    <div style="margin: -20px 0 20px;font-size: 30px;">{{$t( '省份' )}}:{{topFather}} <br /><br />{{$t( '区' )}}：{{fatherName}}</div>
...
<el-table @selection-change="selectionChangeHandle" :data="dataList" border v-loading="dataListLoading"  style="width: 100%;">
      ...
      <el-table-column header-align="center" align="center" :label="$t( '区' )"  prop="name"></el-table-column>
...
<script>
  export default {
    ...
    methods: {
      ...
      update(id){
        this.$refs['dataForm'].validate((valid) => {
          if(valid){
            ...
            this.$http({
            url: this.$http.adornUrl(`/business/addrArea/${!id ? 'save' : 'update'}`),
            method: 'post',
            data: this.$http.adornData({
              id:id||undefined,
              name:this.dataForm.name,
              code:this.dataForm.code,
              cityCode:this.dataForm.provinceCode,
            })
          }).then(({data}) => {
      ...
      init (id,provinceCode,fatherName,topFather) {
        if(fatherName){
...
//        })
      ...
      // 获取数据列表
       getDataList() {
        ...
        this.$http({
          url: this.$http.adornUrl('/business/addrArea/list'),
          method: 'get',
      ...
      deleteHandle (id, userName) {
        ...
        this.$confirm(this.$t( 'messages.template1',[userNames.join(','),id ? this.$t( '删除' ) : this.$t( '批量删除' )] ), this.$t( '提示' ), {
          ...
          this.$http({
            url: this.$http.adornUrl('/business/addrArea/delete'),
            method: 'post',
