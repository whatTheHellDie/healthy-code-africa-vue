<template>
  <div class="mod-menu">
    ...
    <el-table class="show-table" :data="dataList" border style="width: 100%;">
      ...
      <table-tree-column prop="name" header-align="center" treeKey="menuId" :label="$t( '名称' )">
      </table-tree-column>
      <el-table-column prop="sort" header-align="center" align="center"  :label="$t( '排序' )">
      </el-table-column>
      <el-table-column prop="status" header-align="center" align="center"  :label="$t( '状态' )">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.status===0">{{$t( '正常' )}}</el-tag>
          <el-tag type="warning" v-if="scope.row.status===1">{{$t( '禁用' )}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column fixed="right" header-align="center" align="center" width="150" :label="$t( '操作' )">
        <template slot-scope="scope">
          <el-button v-if="isAuth('sys:menu:update')&&scope.row.parentId!==0" type="text" size="small" @click="addOrUpdateHandle(scope.row.menuId)">{{$t( '修改' )}}</el-button>
          <el-button v-if="isAuth('sys:menu:delete')&&scope.row.parentId!==0" type="text" size="small" @click="deleteHandle(scope.row.menuId)">{{$t( '删除' )}}</el-button>
        </template>
      </el-table-column>
    </el-table>
