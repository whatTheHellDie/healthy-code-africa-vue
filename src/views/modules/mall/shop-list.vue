<template>
  <div class="mod-user">
    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <span class="label-search">{{$t( '店铺类型' )}}</span>
      <el-form-item>
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('business:store:save')&&!hasSuperior" type="primary" @click="addOrUpdateHandle()">{{$t( '新增' )}}</el-button>
        <el-button v-if="isAuth('business:store:delete')&&!hasSuperior" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">{{$t( '批量删除' )}}</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;" :class="{noselection:hasSuperior}">
      <el-table-column  type="selection" v-if="!hasSuperior" header-align="center" align="center" width="50">
      </el-table-column>
      ...
      <el-table-column prop="shopName" header-align="center" align="center" :label="$t( '店铺类型' )">
        <template slot-scope="scope">
          <div v-if="scope.row.classify===0">{{$t( '美食' )}}</div>
          <div v-if="scope.row.classify===1">{{$t( '果蔬' )}}</div>
          <div v-if="scope.row.classify===2">{{$t( '超市' )}}</div>
          <div v-if="scope.row.classify===3">{{$t( '医药' )}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="status" header-align="center" align="center" width="150" :label="$t( '店铺区域' )">
        <template slot-scope="scope">
          <div>{{scope.row.oneAddrName+" "+scope.row.twoAddrName+" "+scope.row.threeAddrName}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="detailedAddr" header-align="center" align="center" :label="$t( '门店地址' )">
      </el-table-column>
      ...
      <el-table-column prop="account" header-align="center" align="center" width="200" :label="$t( '管理员账号' )">
      </el-table-column>

      <el-table-column prop="status" header-align="center" align="center" width="150" :label="$t( '密码初始化' )" v-if="!hasSuperior">
        <template slot-scope="scope">
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="180" :label="$t( '操作' )">
        <template slot-scope="scope">
          <div v-if="!hasSuperior">
            <el-button v-if="isAuth('business:store:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{$t( '修改' )}}</el-button>
          ...
          <div v-else>
            <el-button type="text" size="small" @click="bindSuperior(scope.row.id,scope.row.shopName)">{{$t( '绑定' )}}</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      bindSuperior(id,name){//修改轮播图商店id和，名字
