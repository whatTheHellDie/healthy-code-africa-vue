<template>
  <el-dialog :title="!dataForm.id ? $t( '新增' ) : $t( '修改' )" :close-on-click-modal="false" :visible.sync="visible" class="material-store-add">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" :label-width="labelWidth">
      <el-form-item :label="$t( '店铺名称' )" prop="shopName">
        <el-input v-model="dataForm.shopName" :placeholder="$t( '店铺名称' )"></el-input>
      </el-form-item>
      <el-form-item :label="$t( '店铺类型' )" prop="classify">
      ...
      <el-form-item :label="$t( '店铺区域' )" prop="oneAddrId">
       ...
       <el-form-item :label="$t( '店铺地址' )" prop="detailedAddr">
        <el-input v-model="dataForm.detailedAddr" :placeholder="$t( '店铺地址' )"></el-input>
      </el-form-item>
      ...
      <el-form-item :label="$t( 'logo' )" prop="logo">
       <div class="id-card">
              <img class="add-img" v-if="dataForm.logo" :src="dataForm.logo" />
              <div v-else class="add" src="/static/img/ic_add_icon@2x.png"></div>
              <input @change='add_img($event,"logo")' ref="resetFile" accept="image/*;capture=camera" type="file" id="saveImage">
            </div>
      </el-form-item>
