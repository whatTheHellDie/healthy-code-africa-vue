<template>
  <div class="mod-user">
      ...
      <span class="label-search">{{$t( '物资名称' )}}</span>
      <el-form-item>
        <el-input v-model="dataForm.tradeName" :placeholder="$t( '物资名称' )" clearable></el-input>
      </el-form-item>
      <el-form-item>
      ...
      <el-table-column prop="portrait" header-align="center" width="150" align="center" :label="$t( '物资图片' )">
        <template slot-scope="scope">
          <img :src="scope.row.portrait" style="width: 120px;height: 120px;"/>
        </template>
      </el-table-column>
      <el-table-column prop="tradeName" header-align="center" align="center" :label="$t( '物资名称' )"></el-table-column>
      <el-table-column prop="stock" header-align="center" align="center" :label="$t( '库存数量' )">
        <template slot-scope="scope">
          <div>
          {{scope.row.stock}}
          <el-button type="text" size="small" @click="stockAddOrUpdateHandle(scope.row.id,scope.row.tradeName)">{{$t( '添加' )}}</el-button>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="unit" header-align="center" align="center" :label="$t( '物资单位' )">
      </el-table-column>
      <el-table-column prop="quota" header-align="center" align="center" :label="$t( '限购' )">
      </el-table-column>
...
<script>
  import AddOrUpdate from './material-store-add-or-update'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList() {
        ...
        this.$http({
          url: this.$http.adornUrl('/business/material/list'),
          method: 'get',
      ...
      deleteHandle(id, userName) {
        ...
        this.$confirm(this.$t('messages.template1', [userNames.join(','), id ? this.$t('删除') : this.$t('批量删除')]), this.$t('提示'), {
          ...
          this.$http({
            url: this.$http.adornUrl('/business/material/delete'),
            method: 'post',
