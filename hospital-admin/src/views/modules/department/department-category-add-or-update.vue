    ...
    <div>
      <div class="mod-demo-ueditor">
        <el-form ref="dataForm" :rules="dataRules" :model="dataForm" label-width="80px">
          ...
          <el-radio :label="0">{{$t( '科室分类' )}}</el-radio>
          <el-radio :label="1">{{$t( '下级科室分类' )}}</el-radio>
        </el-radio-group>
          </el-form-item>
        ...
        <el-input v-model="dataForm.deptName" v-popover:departmentListPopover :readonly="true" :placeholder="$t( '点击选择上级菜单' )" class="menu-list__input"></el-input>
          ...
          <el-form-item :label="$t( '科室分类' )" prop="addrName">
            <el-input v-model="dataForm.addrName" :placeholder="$t( '科室分类' )" clearable></el-input>
          </el-form-item>
        </el-form>
      </div>
    </div>
