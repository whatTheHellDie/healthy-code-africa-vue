<template>
  ...
  <div class="mod-user">
      ...
      <el-form-item>
         <el-input v-model="dataForm.shopName" :placeholder="$t( '姓名' )" clearable></el-input>
      </el-form-item>
      <span class="label-search">{{$t( '预约号' )}}</span>
      <el-form-item>
         <el-input v-model="dataForm.shopName" :placeholder="$t( '预约号' )" clearable></el-input>
      </el-form-item>
    <div class="inb">
      <span class="label-search">{{$t( '就诊日期' )}}</span>
      <el-date-picker
      ...
      <el-table-column prop="shopName" header-align="center" align="center" :label="$t( '患者姓名/编号' )">
      </el-table-column>
      <el-table-column prop="contacts" header-align="center" align="center" width="200" :label="$t( '性别' )">
        <template slot-scope="scope">
          <div v-if="scope.row.gender===0">{{$t( '男' )}}</div>
          <div v-if="scope.row.gender===1">{{$t( '女' )}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="contacts" header-align="center" align="center" width="200" :label="$t( '科室' )">
        <template slot-scope="scope">
          <div>{{scope.row.oneAddrName+" "+scope.row.twoAddrName+" "+scope.row.threeAddrName}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="contacts" header-align="center" align="center" width="200" :label="$t( '就诊日期' )">
      </el-table-column>
      <el-table-column prop="contacts" header-align="center" align="center" width="200" :label="$t( '就诊医生' )">
      </el-table-column>
      <el-table-column prop="shopName" header-align="center" align="center" :label="$t( '状态' )">
        <template slot-scope="scope">
          <div v-if="scope.row.classify===0">{{$t( '待处理' )}}</div>
          <div v-if="scope.row.classify===1">{{$t( '处理中' )}}</div>
          <div v-if="scope.row.classify===2">{{$t( '已处理' )}}</div>
          <div v-if="scope.row.classify===3">{{$t( '已完成' )}}</div>
        </template>
      </el-table-column>
